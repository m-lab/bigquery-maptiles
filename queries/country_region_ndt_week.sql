...
country_dl AS (
  SELECT
    ...
    CONCAT(client.Geo.country_code, '-', client.Geo.region) AS region,
...
country_daily_per_ip_stats_dl AS (
  ...
  FROM country_dl
  GROUP BY test_date, ip, country, region
),
country_ul AS (
  SELECT
    test_date,
...
country_daily_per_ip_stats_ul AS (
  SELECT test_date, country, region, ip,
...
country_ip_counts AS (
  ...
  all_ips AS (
    SELECT time_period, IP, country, region FROM dl_ips
    UNION ALL (SELECT time_period, IP, country, region FROM ul_ips)
  )
  SELECT 
    time_period,
...
country_asn_counts AS (
  ...
  all_asn AS (
    SELECT time_period, asn, country, region FROM dl_asn
    UNION ALL (SELECT time_period, asn, country, region FROM ul_asn)
  )
  SELECT 
    time_period,
...
JOIN country_dl_sample USING (time_period, country, region)
