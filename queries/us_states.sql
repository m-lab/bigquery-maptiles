...
mlab_dl AS (
  SELECT
    ...
    states.geo_id AS GEOID
  FROM
    `measurement-lab.ndt.unified_downloads` tests,
    `bigquery-public-data.geo_us_boundaries.states` states
  WHERE
...
week_agg AS (
  ...
  SELECT 
    GEOID, main.time_period, dl_count_tests, dl_count_ips, MED_download_Mbps, MED_DL_min_rtt,
    ul_count_tests, ul_count_ips, MED_upload_Mbps
  FROM main, weeks
  WHERE main.time_period = weeks.time_period
)
SELECT * FROM week_agg
JOIN states ON GEOID = states.geo_id
